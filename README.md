...
My scripts that are broken with the new client for https://arras.io due to the new client. They probably won't be able to be rewritten due to the now complexity of the Arras client (except for FOV, which does have a working version for the new client but I won't leak due to it being easy to break and also because I'm not an ass), which has implemented WASM for the protocol, hence uprooting everything before and making it exponetionally harder now, compared to the old base js protocol. This is also why I'm fine with releasing these scripts to the public.
